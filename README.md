...
| Skill | 适用场景 |
|-------|---------|
| [storyboard-master](skills/storyboard-master/) | 想用 AIGC 做视频但没思路 / 要把 IP 改成宣传片 / 需要一份可执行的分镜手册时调用。覆盖项目定位 → 调研 → 创意收敛 → 视觉锁定 → 分镜 → 迭代 → Prompt 输出全流程，工具中立（Seedance / Sora / Kling / Runway / Veo 通用） |
...
ln -s ~/hugo-skills/skills/storyboard-master ~/.claude/skills/storyboard-master
```
...
ln -s ~/hugo-skills/skills/storyboard-master <你的项目>/.claude/skills/storyboard-master
```
...
ln -s ../hugo-skills/skills/storyboard-master skills/storyboard-master
```
...
mv _tmp/skills/storyboard-master .
rm -rf _tmp
```
...
2. 把 `skills/storyboard-master/` 整个文件夹重新压缩成 zip。
...
如果 Claude 自动按"七阶段流程"开始引导（先问 **是什么 / 用途 / 时长** 三问），说明 storyboard-master 已正确加载。

没触发？检查清单：

- [ ] skill 目录名是 `storyboard-master`（**不能改名**，要和 `SKILL.md` frontmatter 里的 `name` 一致）
- [ ] `SKILL.md` 在 skill 根目录，没多嵌一层
- [ ] 软链路径正确：`ls -l ~/.claude/skills/storyboard-master` 能看到指向源文件
...
└── skills/
    └── storyboard-master/
        ...
        └── references/           # 按需加载的支撑文档
            ├── case-dales-immortal.md
            ├── narrative-structures.md
            ├── project-type-adaptations.md
